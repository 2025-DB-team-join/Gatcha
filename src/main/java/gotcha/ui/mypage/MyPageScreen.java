...
setMinimumSize(new Dimension(900, 800)); // 전체 최소 크기
...
infoPanel.setPreferredSize(new Dimension(850, 350)); // 넉넉하게
...
JLabel registeredAt = new JLabel("가입일: " + userInfo.get("registered_at") + " "); // 끝에 공백 추가
...
myReviewBtn.setMargin(new Insets(8, 20, 8, 20)); // 여백만 넉넉히
...
centerPanel.setPreferredSize(new Dimension(850, 540)); // 넉넉하게
...
scroll1.getViewport().setPreferredSize(new Dimension(800, 180));
...
scroll2.getViewport().setPreferredSize(new Dimension(800, 180));
...
scroll3.getViewport().setPreferredSize(new Dimension(800, 180));
...
for (JButton btn : new JButton[]{editInfoBtn, changePwBtn, deleteAccountBtn}) {
